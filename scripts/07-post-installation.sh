...
if [[ "$RUNNING_AS_ROOT" -eq 1 ]]; then
    ...
    if [[ -n "${SUDO_USER:-}" && "$SUDO_USER" != "root" ]] && id "$SUDO_USER" &>/dev/null; then
        # Prefer the account that invoked sudo — the real login user. Falling back
        # to "first UID>=1000" silently targets the wrong account on a machine
        # with more than one regular user.
        CURRENT_USER="$SUDO_USER"
        log_info "Using invoking user (SUDO_USER): $CURRENT_USER"
    elif [[ -z "$existing_users" ]]; then
        ...
        log_warn "SUDO_USER not set — defaulting to first regular account: $CURRENT_USER"
    fi
...
if [[ -f "$VIMRC_PATH" ]] && grep -q "Auto-generated by debian-setup" "$VIMRC_PATH" 2>/dev/null; then
    log_info "Vim already configured with plugins"
else
    log_info "Creating vim configuration..."
    # Preserve a user's own ~/.vimrc (only backs up a real, non-symlink file).
